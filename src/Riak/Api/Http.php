...
class Http extends Api implements ApiInterface
{
    ...
    protected function buildPath()
    {
        ...
        switch (get_class($this->command)) {
            ...
            case 'Basho\Riak\Command\Bucket\Delete':
                ...
                break;
            /** @noinspection PhpMissingBreakStatementInspection */
            case 'Basho\Riak\Command\RiakObject\Fetch':
                $this->headers['Accept'] = '*/*, multipart/mixed';
            case 'Basho\Riak\Command\RiakObject\Store':
            case 'Basho\Riak\Command\RiakObject\Delete':
                $this->path = sprintf('/types/%s/buckets/%s/keys/%s', $bucket->getType(), $bucket->getName(), $key);
                break;
            case 'Basho\Riak\Command\RiakObject\Keys\Fetch':
                $this->headers[static::CONTENT_TYPE_KEY] = static::CONTENT_TYPE_JSON;
            ...
            case 'Basho\Riak\Command\Stats':
                $this->path = '/stats';
                break;
            case 'Basho\Riak\Command\RiakObject\FetchPreflist':
    ...
    protected function prepareRequestHeaders()
    {
        ...
        $object = $this->command->getRiakObject();
        if ($object) {
            if ($object->getVclock()) {
    ...
    protected function parseResponse()
    {
        ...
        switch (get_class($this->command)) {
            ...
            case 'Basho\Riak\Command\RiakObject\Fetch':
            case 'Basho\Riak\Command\RiakObject\Store':
                /** @var Command\RiakObject $command */
                $command = $this->command;
                $objects = (new Api\Http\Translator\ObjectResponse($command, $this->statusCode))
                    ->parseResponse($body, $this->responseHeaders);
                $response = new Command\RiakObject\Response($this->success, $this->statusCode, $this->error, $location, $objects);
                break;

            case 'Basho\Riak\Command\RiakObject\FetchPreflist':
                $response = new Command\RiakObject\Response($this->success, $this->statusCode, $this->error, $location, [new Object(json_decode($body))]);
                break;
            ...
            case 'Basho\Riak\Command\RiakObject\Keys\Fetch':
                ...
                $response = new Command\RiakObject\Keys\Response($this->success, $this->statusCode, $this->error, $keys);
                break;
